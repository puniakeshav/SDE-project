...
clf1 = joblib.load(os.path.join(cdir,'consumer_service/finalized1_model.sav'))
clf2 = joblib.load(os.path.join(cdir,'consumer_service/finalized2_model.sav'))
...
def predict(seg):
    ...
    for ridx, row in enumerate(seg):
        ...
        print(f'prediction on {row[3]} is {result}')
    return all_res

# Test Predict
# text_seg = [['','','facebook','very bad faceb infernece unable to login','']]
